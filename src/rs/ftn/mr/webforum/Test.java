...
//		String xmlAnswer = target.path("rest").path("hello").request().accept(MediaType.TEXT_XML).get(String.class);
		String htmlAnswer = target.path("rest").path("proizvodi").path("hello").request().accept(MediaType.TEXT_HTML).get(String.class);
...
//		System.out.println(xmlAnswer);
	...
	private static URI getBaseURI() {
		return UriBuilder.fromUri("http://localhost:8080/rs.ftn.jersey.webshop").build();
	}
